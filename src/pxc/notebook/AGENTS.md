...
cd src/pxc/notebook/frontend && npm run dev

# Tests
pytest src/pxc/notebook/         # Unit tests
```
...
SQLite database lives at `dist/notebook.db`. Migrations run automatically on server startup via `db.run_migrations()`.

To create a new migration after changing models in `models.py` or `field_store.py`:

```bash
cd src/pxc/notebook && alembic revision -m "description"
```
...
- **Authentication**: email/password with stdlib `hashlib.pbkdf2_hmac` + a DB-backed session token in the `notebook_session` HttpOnly cookie (see `auth.py`, `views/auth.py`). REST endpoints use the `get_current_user` FastAPI dependency; WebSocket endpoints read the cookie directly from `websocket.cookies`.
...
- Never make changes to the upstream `pxc/lib` when making changes that affect only this notebook application.
