# PXC — Notebook Application

The Notebook is a courseware management application built on top of the [PXC](../pxc/) framework. It provides a web interface for organizing courses, pages, and learning activities, with real-time activity execution via WebSockets.
...
│                                              │
│  SQLite + Alembic   │  PXC ActivityRuntime  │
...
**Database** ([db.py](./db.py)): SQLite database at `dist/notebook.db`, with Alembic migrations in [migrations/](./migrations/). Migrations run automatically on startup.
...
- **UserSession** — opaque session token backing the `notebook_session` cookie
...
Field persistence (learner state, scores, etc.) is handled by [field_store.py](./field_store.py), which implements the PXC `FieldStore` interface using three SQLite tables for scalar values, log entries, and sequence counters.
...
- **Components** ([frontend/src/components/](./frontend/src/components/)): course/page/activity lists, sidebar navigation, `<pxc-activity>` web component wrapper
...
```bash
make notebook-frontend-build
```

Run a development server on port 9753:
...
For active frontend development with HMR, you can run `cd src/pxc/notebook/frontend && npm run dev` alongside the FastAPI server.

### LTI integration

It is possible to integrate notebook learning activities in other platforms via LTI 1.3. The `NOTEBOOK_LTI_BASE_URL` environment variable must be defined:

    # In development
    NOTEBOOK_LTI_BASE_URL=http://my-ip-address:9753/lti fastapi dev src/pxc/notebook/app.py --host=0.0.0.0 --port=9753
