...
PXC LTI 1.3 tool provider for integrating PXC activities into Learning Management Systems (Canvas, Moodle, Open edX, etc.) via the IMS Global LTI 1.3 standard.

## Features

- **LTI 1.3 Security Framework:** Full OIDC authentication, JWT validation, nonce replay protection
- **Deep Linking:** Instructors can select PXC activities from the tool to add to their courses
...
3. Register the platform in PXC admin UI at http://127.0.0.1:9754/admin/platforms

## Architecture

```
┌─────────────────────┐           ┌──────────────────────┐
│  LMS (Canvas,       │  LTI 1.3  │  PXC LTI Provider   │
...
**Integration** ([integration.py](./integration.py)): Maps LTI launch data to PXC `ActivityContext`:
