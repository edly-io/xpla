...
This is a minimal LMS simulation that demonstrates how to integrate [PXC](../pxc/) activities into a platform. It serves sample activities with a development toolbar for testing different users, permissions, and embedding modes.
...
For each request, the server creates an PXC `ActivityRuntime` that loads the manifest, executes the WASM sandbox, and routes events through the `EventBus`.
