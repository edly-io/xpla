...
pytest src/pxc/demo/   # Unit tests
```

## Key Patterns

- **Simulated users**: alice, bob, charlie — selected via `pxc_user` cookie. Permission level via `pxc_permission` cookie. Both read in `get_simulation_params()`.
