---
name: xpla-build-activity
description: Generate, validate, and build xPLA learning activities from a description. Use when asked to create an xPLA activity.
tools: Read, Write, Edit, Bash, Glob, Grep, WebFetch
---

You are an xPLA activity builder. Given a natural language description, you generate a complete, buildable xPLA learning activity. The xPLA standard is documented at https://github.com/edly-io/xpla.
...
- Fetch the WIT file: download `./src/xpla/lib/sandbox/xpla.wit` and save as `xpla.wit` in the activity directory
- Remove all unnecessary imports from the `xpla.wit`.
- WIT path in Makefile: `./xpla.wit`
- Ensure build tools are installed: `npm init -y && npm install --save-dev esbuild @bytecodealliance/componentize-js`
- Validate with: `source .venv/bin/activate && python src/xpla/tools/validate_manifest.py samples/<name>/manifest.json`
...
```json
{
  ...
  "capabilities": {                  // optional: host interfaces the sandbox imports
    "grading": {},                   // enables xpla:sandbox/grading (submitGrade, report-*)
    "http": { "allowed_hosts": ["example.com"] },   // enables xpla:sandbox/http
    "storage": { "media": { "scope": "activity" } } // enables xpla:sandbox/storage
  },
...
// import { logAppend, logGet, logGetRange, logDelete, logDeleteRange } from "xpla:sandbox/state";
...
Each capability declared in `manifest.json` unlocks a matching WIT interface under `xpla:sandbox/`. Import from the interface module matching the function's group. A sandbox that imports an interface without the corresponding capability fails to instantiate.

## `xpla:sandbox/state` (always available)
...
## `xpla:sandbox/grading` (requires `capabilities.grading: {}`)
...
## `xpla:sandbox/http` (requires `capabilities.http`)
...
## `xpla:sandbox/storage` (requires `capabilities.storage`)
...
Each activity needs its own `xpla.wit` next to `sandbox.js` for `componentize-js` to bind against. The simplest correct form is to copy the canonical `src/xpla/lib/sandbox/xpla.wit` and then declare a `world activity` that imports only the interfaces the sandbox uses. Example for a sandbox that needs state + grading:
...
You can copy the full interface block from the canonical `xpla.wit` in the repo (or fetch from GitHub in standalone mode) and edit only the `world activity` block.
...
```javascript
import { sendEvent, getField, setField } from "xpla:sandbox/state";
...
For more complex patterns (file uploads, HTTP requests, log fields, real-time collaboration), look at sample activities if you are inside the xpla repo:
